(function (global) {
  const PATTERNS = [
    '11011001100','11001101100','11001100110','10010011000','10010001100','10001001100','10011001000','10011000100','10001100100','11001001000',
    '11001000100','11000100100','10110011100','10011011100','10011001110','10111001100','10011101100','10011100110','11001110010','11001011100',
    '11001001110','11011100100','11001110100','11101101110','11101001100','11100101100','11100100110','11101100100','11100110100','11100110010',
    '11011011000','11011000110','11000110110','10100011000','10001011000','10001000110','10110001000','10001101000','10001100010','11010001000',
    '11000101000','11000100010','10110111000','10110001110','10001101110','10111011000','10111000110','10001110110','11101110110','11010001110',
    '11000101110','11011101000','11011100010','11011101110','11101011000','11101000110','11100010110','11101101000','11101100010','11100011010',
    '11101111010','11001000010','11110001010','10100110000','10100001100','10010110000','10010000110','10000101100','10000100110','10110010000',
    '10110000100','10011010000','10011000010','10000110100','10000110010','11000010010','11001010000','11110111010','11000010100','10001111010',
    '10100111100','10010111100','10010011110','10111100100','10011110100','10011110010','11110100100','11110010100','11110010010','11011011110',
    '11011110110','11110110110','10101111000','10100011110','10001011110','10111101000','10111100010','11110101000','11110100010','10111011110',
    '10111101110','11101011110','11110101110','11010000100','11010010000','11010011100','1100011101011'
  ];

  function encodeToBits(value) {
    const text = value == null ? '' : String(value);
    const codes = [104];
    for (let i = 0; i < text.length; i++) {
      const mapped = Math.max(0, Math.min(95, text.charCodeAt(i) - 32));
      codes.push(mapped);
    }
    let checksum = 104;
    for (let i = 0; i < codes.length; i++) {
      checksum += codes[i] * (i === 0 ? 1 : i);
    }
    const checksumCode = checksum % 103;
    codes.push(checksumCode);
    codes.push(106);
    return codes.map(code => PATTERNS[code] || '').join('');
  }

  function drawToCanvas(canvas, value, label, options = {}) {
    if (!canvas) return null;
    const bits = encodeToBits(value || '');
    const barWidth = options.barWidth || 2;
    const barHeight = options.height || 80;
    const margin = options.margin != null ? options.margin : 10;
    const width = bits.length * barWidth + margin * 2;
    const height = barHeight + 30;
    const ctx = canvas.getContext('2d');
    canvas.width = width;
    canvas.height = height;
    ctx.fillStyle = '#fff';
    ctx.fillRect(0, 0, width, height);
    ctx.fillStyle = '#000';
    for (let i = 0; i < bits.length; i++) {
      if (bits[i] === '1') {
        ctx.fillRect(margin + i * barWidth, 5, barWidth, barHeight);
      }
    }
    const display = options.displayValue !== false;
    if (display) {
      ctx.font = '14px system-ui, sans-serif';
      ctx.textAlign = 'center';
      ctx.fillText(label || value || '', width / 2, barHeight + 22);
    }
    return canvas;
  }

  function drawToSvg(target, value, options = {}) {
    const element = typeof target === 'string' ? document.querySelector(target) : target;
    if (!element) return null;
    const bits = encodeToBits(value || '');
    const barWidth = options.barWidth || 2;
    const barHeight = options.height || 80;
    const margin = options.margin != null ? options.margin : 10;
    const display = options.displayValue !== false;
    const width = bits.length * barWidth + margin * 2;
    const height = barHeight + (display ? 18 : 0) + margin * 2;

    while (element.firstChild) element.removeChild(element.firstChild);
    element.setAttribute('xmlns', 'http://www.w3.org/2000/svg');
    element.setAttribute('width', width);
    element.setAttribute('height', height);

    const group = document.createElementNS('http://www.w3.org/2000/svg', 'g');
    group.setAttribute('transform', `translate(${margin},${margin})`);
    for (let i = 0; i < bits.length; i++) {
      if (bits[i] === '1') {
        const rect = document.createElementNS('http://www.w3.org/2000/svg', 'rect');
        rect.setAttribute('x', i * barWidth);
        rect.setAttribute('y', 0);
        rect.setAttribute('width', barWidth);
        rect.setAttribute('height', barHeight);
        rect.setAttribute('fill', '#000');
        group.appendChild(rect);
      }
    }
    element.appendChild(group);

    if (display) {
      const text = document.createElementNS('http://www.w3.org/2000/svg', 'text');
      text.textContent = label || value || '';
      text.setAttribute('x', margin + 2);
      text.setAttribute('y', barHeight + margin + 12);
      text.setAttribute('font-size', '12');
      text.setAttribute('font-family', 'system-ui, -apple-system, "Segoe UI", sans-serif');
      element.appendChild(text);
    }
    return element;
  }

  function toDataUrl(value, label, options = {}) {
    const canvas = document.createElement('canvas');
    drawToCanvas(canvas, value, label, options);
    return canvas.toDataURL('image/png');
  }

  global.Code128 = { drawToCanvas, drawToSvg, toDataUrl };
})(typeof window !== 'undefined' ? window : globalThis);
