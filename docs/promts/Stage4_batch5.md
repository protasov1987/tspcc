...
3. Сохранить текущие правила:
   ...
   - как сохраняется номер заказа на производство в `mainMaterials`
...
- stage transition `APPROVED -> WAITING_INPUT_CONTROL/PROVIDED` сохраняется
...
npm run version:bump -- --change "Provision карточек переведен на отдельную команду"
...
3. Проверь:
   ...
   - номер заказа на производство сохранился как раньше
