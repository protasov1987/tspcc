...
4. Если input control зависит от attachment state, не переносить file-domain в этот batch;
   использовать только уже существующие данные и safe integration point.
...
npm run version:bump -- --change "Input control карточек переведен на отдельные команды без потери маршрута"
...
6. Если input control перестал работать или начал требовать действия с файлами, которых раньше не требовал, batch не закрыт.
