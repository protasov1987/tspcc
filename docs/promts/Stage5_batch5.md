...
- после upload/delete/resync не терять обязательные file-linked side effects в самой карточке
- не переписывать заново Stage 4

Что нужно сделать:
1. Найти side effects, завязанные на file-domain карточки, которые реально живут в card model:
   - `attachments[]`
   - `inputControlFileId`
   - `filesCount`
   - `rev`
...
4. Проверить, что client-side patch path вроде `applyFilesPayloadToCard()` остается согласованным с новым server contract.
5. Сохранить live update compatibility (`card.files-updated`, `cards-live`), но не полагаться только на нее для локальной корректности после file write.
6. Не менять business semantics input control за пределами file linkage.
7. Shared attachment store для `TECH_SPEC`, `TRPN`, `PARTS_DOCS` можно трогать только на уровне совместимости общего контракта, но без миграции production / receipts UI flows.
8. Не начинать Stage 6.
...
- не переносить сюда production migration из-за `TECH_SPEC` / `TRPN`
- не переносить сюда receipts migration из-за `PARTS_DOCS`
...
- локальная карточка и live update не начинают расходиться по `attachments[]`, `inputControlFileId`, `filesCount`, `rev`
...
8. Если этот файл участвовал в input control, проверь, что ссылка на него тоже обновилась корректно.
