...
test.describe.serial('workspace card-file action contexts', () => {
  ...
  test('shows selected item names in attachments modal after workspace thermogram upload', async ({ page }) => {
    await loginAsAbyss(page, { startPath: '/workspace' });
    await waitUsableUi(page, '/workspace');
    ...
    await page.selectOption('#workspace-transfer-docs-id', 'Терм');
    await page.setInputFiles('#workspace-transfer-docs-files', {
      name: 'workspace-therm-items-label.png',
      mimeType: 'image/png',
      buffer: Buffer.from('iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mP8/x8AAwMCAO2pL6kAAAAASUVORK5CYII=', 'base64')
    });
    await expect(page.locator('#workspace-transfer-docs-list')).toContainText('workspace-therm-items-label.png');
    ...
    const fileRow = page.locator('#attachments-modal tbody tr').filter({
      hasText: 'Терм_workspace-therm-items-label.png'
    }).first();
    await expect(fileRow).toBeVisible();
