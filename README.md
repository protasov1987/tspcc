...
- `js/app.*.js` — функциональность приложения, работа с серверным API и генерацией QR-кодов (строковые значения штрихкодов сохраняются в поле `barcode`, скрипты разделены по секциям).
