...
Локальное веб-приложение для управления маршрутными картами, маршрутами операций и трекингом выполнения. Все данные хранятся на сервере в файле-базе `data/database.json`, поэтому ими могут пользоваться несколько пользователей из разных браузеров и устройств, подключённых к одному экземпляру приложения.

## Структура
- `index.html` — основной каркас страницы и разметка.
- `style.css` — стили оформления.
- `js/app.*.js` — функциональность приложения, работа с серверным API и генерацией QR-кодов (QR ID хранится в поле `qrId`, legacy-значения штрихкодов остаются в поле `barcode`), скрипты разделены по секциям).
...
- `data/database.json` — файл-база, где сервер хранит текущее состояние карт, операций и подразделений.

## Установка и запуск локально
1. Убедитесь, что установлен Node.js 18+.
2. Установите зависимости (используется пакет `qrcode` для генерации QR-кодов):
   ```bash
   npm install
   ```
3. Запустите сервер:
...
- Каждая карта отображает статусную плашку рядом с кнопкой QR-кода. Завершённые карты можно перенести в архив кнопкой «Перенести в архив».
- Во время выполнения операций можно добавлять комментарии в столбце «Комментарии»; для завершённых операций поле блокируется.
- Архив доступен на отдельной вкладке с собственным поиском и фильтром по статусу. Карты в архиве доступны только для просмотра, но их можно «Повторить» — создаётся копия со свежим QR-кодом, статусом «Не запущена» и сброшенными таймерами.
...
- В окне показываются QR ID и реквизиты карты, состояние при создании, полная таблица изменений (когда менялись поля, операции, статусы, исполнители, файлы) и сводная таблица операций в том же порядке столбцов, что и во вкладке квитанций.
...
- Кнопка «Печать сводной таблицы» формирует лист A4 в альбомной ориентации с названием карты, QR ID, номером заказа и текущей сводной таблицей.
